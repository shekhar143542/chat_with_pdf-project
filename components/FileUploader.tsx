...
import {CircleArrowDown, RocketIcon,CheckCircleIcon, SaveIcon, HammerIcon} from 'lucide-react'
...
const router = useRouter();
  ...
  useEffect(() => {
    console.log("fileId updated:", fileId);
    if(fileId){
      console.log("Redirecting to:", `/dashboard/files/${fileId}`);
...
{/* Loading... tomorrow */}
    ...
    {uploadInProgress && (
      ...
      {/* @ts-ignore */}
        <p className='text-indigo-700 animate-pulse'>{status}</p>
      </div>
    )}
     ...
     { !uploadInProgress && ( <div 
        ...
        className={`p-10 border-2 border-dashed mt-10 w-[90%]
          text-indigo-700 border-indigo-700 rounded-lg h-96 flex justify-center items-center 
